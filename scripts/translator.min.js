	...
	class EOTranslator {
		...
		translateElement(DOMElement, lang) {
			if (DOMElement) {
                ...
                const text = this.translate(input, { lang: language, fallback, params });

                if(html) {
                    DOMElement.innerHTML = text;
					const event = new CustomEvent('eo-translator:translateDom', {
						bubbles: true,
						cancelable: true
					});
                ...
                const el = Array.from(DOMElement.childNodes).find(n => n.nodeType === Node.TEXT_NODE);
                if (el) {
                    el.textContent = text;
                }
